...
// Función para enviar mensajes a la consola del .bat
...
ws.onmessage = (event) => {
    try {
        ...
        remoteLog("Datos recibidos: " + event.data);
...
function procesarGiroRuleta(categoria, baneosSolicitadosWs) {
    remoteLog("¡Comenzando giro para categoria: " + categoria + "!");
...
function procesarReroll(slotStr) {
    ...
    if (indiceSlotActual < 4) {
        remoteLog("Reroll denegado: Ruleta en curso.");
        return;
    }

    const slotTarget = parseInt(slotStr) - 1; // Convertir 1-4 a 0-3
    if (slotTarget >= 0 && slotTarget <= 3) {
        remoteLog("Iniciando Reroll para slot " + slotStr);
...
function cambiarIdiomaSistema(nuevoIdioma) {
    if (nuevoIdioma === "esp" || nuevoIdioma === "eng") {
        ...
        remoteLog("Idioma actualizado a: " + idiomaActual);
...
function ejecutarRerollSeleccionado() {
    if (indiceSlotActual < 4) {
        remoteLog("Reroll denegado: Debes girar la ruleta completa primero.");
        return;
    }
