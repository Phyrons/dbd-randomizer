...
function abrirPanelFiltros() {
    ...
    sinPerkBox.innerText = `NO PERK: ${sinPerkHabilitadoPorCategoria[categoriaActual] ? 'ON' : 'OFF'}`;
    ...
    sinPerkBox.onclick = () => {
        ...
        sinPerkBox.innerText = `NO PERK: ${activo ? 'ON' : 'OFF'}`;
...
function construirPista(listaBase) {
    ...
    listaFinal.forEach(p => {
        ...
        if (p.sinPerk) {
            ...
            div.innerHTML = `<span>No Perk</span>`;
        } else {
